...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        _previous_high = previous_high
        _previous_score = previous_score
        if who == 0:
            gain = score0 - _previous_score
        else:
            gain = score1 - _previous_score

        _previous_score += gain
        
        if gain > _previous_high:
            _previous_high = gain
            if gain == 1:
                print('1 point! That\'s the biggest gain yet for Player', who)
            else:
                print(gain, 'points! That\'s the biggest gain yet for Player', who)
        return announce_highest(who, _previous_high, _previous_score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while i <= 10:
        if average(i, dice) > max_aver_score:
            max_aver_score = average(i, dice)
            result = i
        i += 1
    return result
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(tmp, opponent_score) and (opponent_score - score) >= margin:
        return 0
